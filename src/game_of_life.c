...
int main() {
    ...
    if (flag == 0) {
        printf("Выберите скорость (от 1 до 5) : ");
        scanf("%d", &score);
    }

    while (flag == 0) {
        int seed = speed(&flag, score);
        if ((9999 < seed) && (seed < 500001)) {
...
void read_file(int a[HEIGHT][WIDTH], int *flag) {
    int figure = 1;
    printf("Выберите начальную инициализацию (от 1 до 5) : ");
